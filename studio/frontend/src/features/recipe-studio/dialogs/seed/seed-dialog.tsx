...
export function SeedDialog({ config, onUpdate, open }: SeedDialogProps): ReactElement {
  ...
  const gsspEndpointId = `${config.id}-gssp-endpoint`;
  const gsspPathId = `${config.id}-gssp-path`;
  const gsspAuthTokenId = `${config.id}-gssp-auth-token`;
  const gsspCorrelationId = `${config.id}-gssp-correlation-id`;
  const gsspApplicationId = `${config.id}-gssp-application-id`;
  const gsspSoeidId = `${config.id}-gssp-soeid`;
  const gsspModelNameId = `${config.id}-gssp-model-name`;
  const gsspMaxTokensId = `${config.id}-gssp-max-tokens`;
  const gsspAuthorizationCoinId = `${config.id}-gssp-authorization-coin`;
        ...
        <div className="space-y-3">
          ...
          {mode === "unstructured" && (
            <div className="space-y-3 rounded-xl corner-squircle border border-border/60 p-3">
              <FieldLabel
                label="GSSP integration"
                hint="These headers and auth values are sent by backend when running this recipe."
              />
              <div className="grid gap-2">
                <div className="grid gap-1.5">
                  <FieldLabel label="GSSP endpoint" htmlFor={gsspEndpointId} />
                  <Input
                    id={gsspEndpointId}
                    className="nodrag"
                    value={config.gssp_endpoint ?? ""}
                    placeholder="https://...nsroot.net"
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_endpoint: event.target.value,
                      })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel label="GSSP path" htmlFor={gsspPathId} />
                  <Input
                    id={gsspPathId}
                    className="nodrag"
                    value={
                    ...
                    onChange={(event) =>
                      onUpdate({ gssp_enabled: true, gssp_path: event.target.value })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel label="Auth token" htmlFor={gsspAuthTokenId} />
                  <Input
                    id={gsspAuthTokenId}
                    className="nodrag"
                    value={config.gssp_auth_token ?? ""}
                    placeholder="Bearer token"
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_auth_token: event.target.value,
                      })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel
                    label={
                      <>
                        x-correlation-id <span className="text-red-500">*</span>
                      </>
                    }
                    htmlFor={gsspCorrelationId}
                  />
                  <Input
                    id={gsspCorrelationId}
                    className="nodrag"
                    value={config.gssp_x_correlation_id ?? ""}
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_x_correlation_id: event.target.value,
                      })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel
                    label={
                      <>
                        x-application-id <span className="text-red-500">*</span>
                      </>
                    }
                    htmlFor={gsspApplicationId}
                  />
                  <Input
                    id={gsspApplicationId}
                    className="nodrag"
                    value={config.gssp_x_application_id ?? ""}
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_x_application_id: event.target.value,
                      })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel
                    label={
                      <>
                        x-soeid <span className="text-red-500">*</span>
                      </>
                    }
                    htmlFor={gsspSoeidId}
                  />
                  <Input
                    id={gsspSoeidId}
                    className="nodrag"
                    value={config.gssp_x_soeid ?? ""}
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_x_soeid: event.target.value,
                      })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel
                    label="x-Authorization-Coin"
                    htmlFor={gsspAuthorizationCoinId}
                  />
                  <Input
                    id={gsspAuthorizationCoinId}
                    className="nodrag"
                    value={config.gssp_x_authorization_coin ?? ""}
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_x_authorization_coin: event.target.value,
                      })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel
                    label="x-model-name"
                    htmlFor={gsspModelNameId}
                  />
                  <Input
                    id={gsspModelNameId}
                    className="nodrag"
                    value={config.gssp_x_model_name ?? ""}
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_x_model_name: event.target.value,
                      })
                    }
                  />
                </div>
                <div className="grid gap-1.5">
                  <FieldLabel
                    label="x-max-tokens"
                    htmlFor={gsspMaxTokensId}
                  />
                  <Input
                    id={gsspMaxTokensId}
                    className="nodrag"
                    inputMode="numeric"
                    value={config.gssp_x_max_tokens ?? ""}
                    onChange={(event) =>
                      onUpdate({
                        gssp_enabled: true,
                        gssp_x_max_tokens: event.target.value,
                      })
                    }
                  />
                </div>
