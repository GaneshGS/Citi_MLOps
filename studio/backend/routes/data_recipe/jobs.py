...
def _inject_local_providers(recipe: dict[str, Any], request: Request) -> None:
    ...
    if local_names & referenced_providers:
        raise ValueError(
            "Local Studio inference (model providers with is_local=true) is not "
            "available in this build. Use an OpenAI-compatible endpoint from Citi "
            "Model Garden or GSSP instead."
        )
