...
Pure functions and a stateful stream emitter for format conversion; optional
PIL-based image normalization for vision payloads.
"""
...
def _normalize_anthropic_openai_images(
    ...
    ``image_url`` parts with base64 data URLs to PNG (for downstream servers).
    Mutates ``openai_messages`` in place. Returns True when any image part was seen.
    """
